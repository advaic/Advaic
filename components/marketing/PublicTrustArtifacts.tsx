...
export default function PublicTrustArtifacts({
  ...
  const visibleArtifacts = rail
    ? PUBLIC_TRUST_ARTIFACTS.slice(0, 3)
    : compact
      ? PUBLIC_TRUST_ARTIFACTS.slice(0, 4)
      : PUBLIC_TRUST_ARTIFACTS;
  ...
  return (
    <article className={wrapperClass} data-tour={dataTour}>
      ...
      <div
        className={`mt-4 grid gap-3 ${rail ? "grid-cols-1" : compact ? "grid-cols-3" : "md:grid-cols-3"}`}
        ...
        {PUBLIC_TRUST_SUMMARIES.map((item) => (
          <article
            ...
            className={`rounded-2xl bg-[var(--surface-2)] ring-1 ring-[var(--border)] ${
              rail ? "px-4 py-3.5" : compact ? "px-3 py-3" : "px-4 py-4"
            }`}
          >
            {rail ? (
              <div className="flex items-baseline justify-between gap-4">
                <p className="text-2xl font-semibold tracking-[-0.02em] text-[var(--text)]">{item.value}</p>
                <p className="text-right text-xs font-semibold leading-5 text-[var(--text)]">{item.title}</p>
              </div>
            ) : (
              <>
      ...
      <div
        className={`mt-4 grid gap-3 ${rail ? "grid-cols-1" : compact ? "grid-cols-2" : "md:grid-cols-2 xl:grid-cols-3"}`}
        ...
        {visibleArtifacts.map((item) => (
          <article
            ...
            className={`rounded-2xl bg-white ring-1 ring-[var(--border)] ${
              rail ? "px-4 py-4" : compact ? "px-3 py-3" : "px-4 py-4"
            }`}
