...
function inferObjectFocus(snippets: string[]) {
...
function inferTone(snippets: string[]) {
...
function inferReadiness(activeListingsCount: number | null, snippets: string[]) {
...
export async function POST(
  ...
  const docsFlow = snippets.find((snippet) =>
    /unterlagen|exposé|dokument|bonit/i.test(snippet),
  );
  ...
  if (!prospect.source_checked_at) updates.source_checked_at = new Date().toISOString().slice(0, 10);
  if ((!prospect.linkedin_search_url || String(prospect.linkedin_search_url).trim() === "") && !prospect.linkedin_url) {
  ...
  if ((!prospect.object_focus || prospect.object_focus === "gemischt") && inferredFocus !== "gemischt") {
    updates.object_focus = inferredFocus;
  }
  if (!prospect.brand_tone && inferredTone) {
    updates.brand_tone = inferredTone;
  }
  if (!prospect.automation_readiness && inferredReadiness) {
    updates.automation_readiness = inferredReadiness;
  }
  if (!prospect.response_promise_public && responsePromise) {
    updates.response_promise_public = normalizeLine(responsePromise, 170);
  }
  if (!prospect.appointment_flow_public && appointmentFlow) {
    updates.appointment_flow_public = normalizeLine(appointmentFlow, 170);
  }
  if (!prospect.docs_flow_public && docsFlow) {
    updates.docs_flow_public = normalizeLine(docsFlow, 170);
  }
  if (!prospect.personalization_evidence && autoEvidence) {
    updates.personalization_evidence = autoEvidence;
  }
  if (!prospect.primary_pain_hypothesis && !prospect.pain_point_hypothesis) {
  ...
  if (Object.keys(updates).length > 0) {
    ...
    if (updateErr) {
      return NextResponse.json(
        { ok: false, error: "crm_enrich_update_failed", details: updateErr.message },
        { status: 500 },
      );
    }
  }
  ...
  if (noteParts.length > 0) {
    await (supabase.from("crm_research_notes") as any).insert({
